...
def calcul():
    if request.method == "POST":
        ...
        notes = []
        FR_ecrit = request.form["FR_ecrit"]
        FR_oral = request.form["FR_oral"]
        ...
        for i in range(1, 3):
            for j in ["sport", "philo", "phch", "svt", "math", "lv1", "hg", "lv2", "spe", "option"]:
                notes.append(request.form[j + str(i)])
        if not notes[9]:
            option = False
        else:
            option = True
        ...
        for i in range(len(notes) // 2 + 1):
            try:
                moyenne_m.append((float(notes[i]) + float(notes[i + 10])) / 2)
            except:
                break
        # calcul
        ...
        if option:
            moyenne_tot = (moyenne_m[4] * coefs[4] + moyenne_m[2] * coefs[2] + moyenne_m[3] * coefs[3] + moyenne_m[6] * coefs[6] + moyenne_m[5] * coefs[5] + moyenne_m[7] * coefs[0] +
                           moyenne_m[1] * coefs[1] + float(FR_oral) * coefs[0] + float(FR_ecrit) * coefs[0] + ((float(TPE) - 10) if float(TPE) > 10 else 0) * coefs[0] + ((moyenne_m[9] - 10) if moyenne_m[9] > 10 else 0) * coefs[0] + moyenne_m[0] * coefs[0]) / somme_des_coefs
        else:
            moyenne_tot = (moyenne_m[4] * coefs[4] + moyenne_m[2] * coefs[2] + moyenne_m[3] * coefs[3] + moyenne_m[6] * coefs[6] + moyenne_m[5] * coefs[5] + moyenne_m[7] * coefs[0] +
                           moyenne_m[1] * coefs[1] + float(FR_oral) * coefs[0] + float(FR_ecrit) * coefs[0] + ((float(TPE) - 10) if float(TPE) > 10 else 0) * coefs[0] + moyenne_m[0] * coefs[0]) / somme_des_coefs
